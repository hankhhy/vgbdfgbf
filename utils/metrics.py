...
def plot_mc_curve(px, py, save_dir=Path('mc_curve.png'), names=(), xlabel='Confidence', ylabel='Metric'):
    ...
    fig, ax = plt.subplots(1, 1, figsize=(9, 6), tight_layout=True)
    max_indx = 300  # max_indx=300对应conf=0.3时的P、R取值
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
...
def compute_ap(recall, precision):
    ...
    temp1 = np.flip(mpre)
    temp2 = np.maximum.accumulate(np.flip(mpre))
    mpre = np.flip(np.maximum.accumulate(np.flip(mpre)))

    # Integrate area under curve
    method = 'searchsorted'  # methods: 'continuous', 'interp'
    if method == 'interp':
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir=Path(), names=(), eps=1e-16, prefix=''):
    ...
    for ci, c in enumerate(unique_classes):
        ...
        #保存本地npy
        # np.save("./runs/detect/train/val/tp.npy", tp)
